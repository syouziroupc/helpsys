...
Production `wrangler.jsonc` contains an `AI` binding and no persistent data-store binding. The multimodal Worker calls `env.AI.run()` directly. The Safe desktop build therefore remains blocked from public distribution because processing geography and model-license evidence are not yet fully resolved, not because the local Privacy Gate is incomplete.
