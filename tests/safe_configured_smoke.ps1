...
New-Item -ItemType Directory -Force -Path 'smoke-bin/unified-test' | Out-Null

dotnet build src/HelpSys.Desktop/HelpSys.Desktop.csproj -c Release --no-restore -p:TestBuild=true -o smoke-bin/unified-test
if ($LASTEXITCODE -ne 0) { throw 'Unified CI test build failed.' }
$testExe = Resolve-Path 'smoke-bin/unified-test/HelpSys.exe'
...
try {
  ...
  $helpSys = Start-Process $testExe -PassThru
  Start-Sleep -Seconds 5
  if ($helpSys.HasExited) { throw 'Unified HelpSys CI test build exited during startup.' }
  ...
  if ($null -eq $request -or $null -eq $guide -or $null -eq $voice -or $null -eq $commander -or $null -eq $privacy -or $null -eq $state) {
    throw 'Unified HelpSys CI test build did not expose required UI controls.'
  }
  if (-not $voice.Current.IsEnabled) { throw 'Unified HelpSys must retain explicit Voice input.' }
  if (-not $commander.Current.IsEnabled) { throw 'Unified HelpSys must retain Commander control.' }
  if ($state.Current.Name -notlike '*統合版*') { throw "Unified HelpSys did not visibly identify the unified profile. State: $($state.Current.Name)" }
  ...
  while (-not (Test-Path 'artifacts/mock-last-request.json') -and [DateTime]::UtcNow -lt $deadline) {
    if ($helpSys.HasExited) { throw 'Unified HelpSys CI test build exited before reaching its compile-time test-only loopback endpoint.' }
    Start-Sleep -Milliseconds 200
  }

  if (-not (Test-Path 'artifacts/mock-last-request.json')) {
    Save-Screenshot 'helpsys-unified-configured-failure.png'
    throw 'Unified CI test build did not reach its compile-time test-only loopback endpoint.'
  }

  $diagnostics = Get-Content 'artifacts/mock-last-request.json' -Raw -Encoding UTF8 | ConvertFrom-Json
  if ($diagnostics.path -notin @('/v2/plan','/v1/guide','/v1/quality-guide')) { throw "Unified test build used an unexpected route: $($diagnostics.path)" }
  ...
  if ([int]$diagnostics.foregroundProcessId -ne $target.Id) { throw "Unified test build analyzed the wrong process: $($diagnostics.foregroundProcessId), expected $($target.Id)." }

  Start-Sleep -Seconds 2
  if ($state.Current.Name -like '*プライバシー保護のため画面解析を一時停止中*') {
    Save-Screenshot 'helpsys-unified-configured-failure.png'
    throw 'Unified test build incorrectly entered Privacy Mode on the benign smoke surface.'
  }

  Save-Screenshot 'helpsys-unified-configured-guidance.png'
  Write-Host 'HelpSys unified CI-only loopback E2E smoke passed.'
}
finally {
