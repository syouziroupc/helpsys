...
const SCREEN_ROUTES = new Set(['/v1/outlaw-plan', '/v1/quality-guide', '/v1/guide', '/v1/vision-guide']);
...
export default {
  async fetch(request, env, ctx) {
    ...
    if (url.pathname === '/v1/outlaw-plan') {
      ...
      if (hasImage) {
        const routedUrl = new URL(screenRequest.url);
        routedUrl.pathname = '/v1/quality-guide';
        return quality.fetch(new Request(routedUrl.toString(), screenRequest), privateEnv, ctx);
      }
      const routedUrl = new URL(screenRequest.url);
      routedUrl.pathname = '/v1/guide';
      return base.fetch(new Request(routedUrl.toString(), screenRequest), privateEnv, ctx);
    }
